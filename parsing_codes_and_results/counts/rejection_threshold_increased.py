...
class VideoRatingParser:
    def __init__(self, csv_path, sample_csv, backup_csv, gold_csv, base_path, row_index=0, print_analysis=True):
        ...
        # self.max_same_pattern_count =50  # maximum allowed videos with same attribute pattern (50->14 (L) for other batches)
...
def process_batch(csv_path, sample_csv, backup_csv, gold_csv, base_path=None, clear_existing=False, print_analysis=True):
    ...
    # Clear global lists
    global rejected_reason, rejected_worker_id, rejected_hit_id, rejected_assgn_id, rejected_details, participated_worker_id
    rejected_reason = []
    rejected_worker_id = []
    rejected_hit_id = []
    rejected_assgn_id = []
...
# def process_all_csv_files_in_directory(input_folder, sample_csv, backup_csv, gold_csv, clear_existing=False, print_analysis=True):
...
#     base_path = os.path.join(parent_dir, f'{input_folder_name}_secondparse')
...
#     print(f"\n{'='*80}")
#     print(f"Completed processing all CSV files in {input_folder}")
...
if __name__ == '__main__':
    ...
    print("\nBatch-wise Results:")
    print("-" * 60)
    print(f"{'Batch':<6} {'Submissions':<12} {'Rejections':<12} {'Rate (%)':<10}")
    print("-" * 60)
    for batch in batches:
        submissions = batch_total_submissions.get(batch, 0)
        rejections = batch_rejection_counts.get(batch, 0)
        rate = (rejections / submissions * 100) if submissions > 0 else 0
        print(f"{batch:<6} {submissions:<12} {rejections:<12} {rate:<10.1f}")
    
    print("-" * 60)
    print(f"{'TOTAL':<6} {overall_total_submissions:<12} {overall_total_rejections:<12} {overall_rejection_rate:<10.1f}")
    print("-" * 60)
    ...
    # Find batch with highest/lowest rejection rates
    batch_rates = {batch: (batch_rejection_counts[batch] / batch_total_submissions[batch] * 100) 
                   if batch_total_submissions[batch] > 0 else 0 for batch in batches if batch in batch_total_submissions}
    
    if batch_rates:
        highest_rate_batch = max(batch_rates, key=batch_rates.get)
        lowest_rate_batch = min(batch_rates, key=batch_rates.get)
    
        print(f"\n🎯 SUMMARY:")
        print(f"   • Total Batches Processed: {len(batches)}")
        print(f"   • Total Submissions: {overall_total_submissions}")
        print(f"   • Total Rejections: {overall_total_rejections}")
        print(f"   • Overall Rejection Rate: {overall_rejection_rate:.2f}%")
        print(f"   • Average Rejections per Batch: {overall_total_rejections/len(batches):.1f}")
        print(f"   • Highest Rejection Rate: Batch {highest_rate_batch} ({batch_rates[highest_rate_batch]:.1f}%)")
        print(f"   • Lowest Rejection Rate: Batch {lowest_rate_batch} ({batch_rates[lowest_rate_batch]:.1f}%)")
    print("="*80)
