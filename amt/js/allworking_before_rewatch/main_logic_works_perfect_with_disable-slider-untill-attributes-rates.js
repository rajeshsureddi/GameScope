function dbg() {
  ...
  function show_slider(vids) {
    ...
    // Clear attribute radios
    $('input[name="sharpness"], input[name="artifacts"], input[name="color"], input[name="immersion"]')
      .prop('checked', false);
    // Clear modal checkboxes
    $('#attributes').find('input[type=checkbox]').prop('checked', false);
    // Hide any old popover and disable submit button
    $('#sub_butA').popover('hide').prop('disabled', true);
  
    // ── 2. Prepare initial slider value ──────────────────────────
    ...
    $('#buttonA, #report_video').hide();
  
    // ── 3. Show everything but keep the slider disabled ──────────
    $('#main-slider').show();
    // Ensure slider info & steps visible
    $('#slider_info, #points, .steps').show();
    // Disable slider until attributes done
    $('#points').prop('disabled', true);
  
    // ── 4. When all four radios are checked, enable the slider ───
    $('input[name="sharpness"], input[name="artifacts"], input[name="color"], input[name="immersion"]')
      .off('change.attrCheck')
      .on('change.attrCheck', function() {
        const allRated = ['sharpness','artifacts','color','immersion']
          .every(name => document.querySelector(`input[name="${name}"]:checked`));
        if (!allRated) return;
        // Enable slider & move focus
        $('#points').prop('disabled', false).focus();
        // Unbind this handler
        $('input[name="sharpness"], input[name="artifacts"], input[name="color"], input[name="immersion"]')
          .off('change.attrCheck');
      });
  
    // ── 5. Bind the slider change to show submit (unchanged) ─────
    $('#points')
      .off('change.submit')
...
default:
  ...
  const required = [
    {name:'sharpness', label:'Clarity & Sharpness'},
    {name:'artifacts',  label:'Pixelation & Blockiness'},
    {name:'color',      label:'Color Accuracy & Visibility'},
    {name:'immersion',  label:'Immersive Game Experience'}
  ];
  const missing = required
    .filter(a => !document.querySelector(`input[name="${a.name}"]:checked`))
    .map(a => a.label);
  if (missing.length) {
    const msg = 'Please rate: ' + missing.join(', ');
    $('#sub_butA')
      .attr('data-content', msg)
      .popover('show');
    return;
  }
    // …then the rest of your original submitAnswer logic follows unchanged
