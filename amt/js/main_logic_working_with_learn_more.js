function dbg() {
  ...
  function show_slider(vids) {
    ...
    // 1. Clear the four new radio groups
  ...
  function openAttributePage(attribute) {
    ...
    const pages = {
      ...
      'color': 'https://gaming-vqa.s3.us-east-2.amazonaws.com/example-samples/amt_intro/color.htm',
  ...
  function submitAnswer() {
    ...
    if (phase === 'train') {
      ...
      if (learnMoreClicks.sharpness < minClicks) missingClicks.push('Clarity & Sharpness');
      if (learnMoreClicks.artifacts < minClicks) missingClicks.push('Pixelation & Blockiness');
      if (learnMoreClicks.color < minClicks) missingClicks.push('Color Accuracy & Visibility');
    ...
    ['sharpness','artifacts','color','immersion'].forEach(function(name) {
